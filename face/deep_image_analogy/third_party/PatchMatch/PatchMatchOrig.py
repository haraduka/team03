...
class PatchMatch(object):
    ...
    def visualize(self):
        ...
        content14 = cv2.imread('in14A.png')
        style14 = cv2.imread('in14B.png')
        out_content = np.zeros_like(content14)
        out_style = np.zeros_like(style14)
        ...
        for i in range(nnf.shape[0]):
            for j in range(nnf.shape[1]):
                ...
                print(nnf[i, j], end='')
                out_style[i, j] = content14[nnf[i, j][1], nnf[i, j][0]]
                out_content[i, j] = style14[nnf[i, j][1], nnf[i, j][0]]
            print()
        cv2.imwrite('out_content.png', out_content)
        cv2.imwrite('out_style.png', out_style)
        return img
            ...
            while ay != yend:
                ...
                while ax != xend:
                    ...
                    if ax - xchange < a_cols and ax - xchange >= 0:
                        vp = self.nnf[ay, ax - xchange] # 一つ左のnnfを使ってサーチ
                        xp = vp[0] + xchange
                        yp = vp[1]
                    ...
                    if abs(ay - ychange) < a_rows and ay - ychange >= 0:
                        vp = self.nnf[ay - ychange, ax] # 一つ上のnnfを使ってサーチ
                        xp = vp[0]
                        yp = vp[1] + ychange
